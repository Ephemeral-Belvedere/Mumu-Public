...
class DraftPhase(Enum):
    ...
    ASSASSIN_SURVIVAL = "assassin_survival"  # Dice roll phase when too many restricted servants
    DISHWASHING = "dishwashing"  # Losers pick new servants
...
class DraftSession:
    ...
    servant_tiers: Dict[str, List[str]] = field(default_factory=lambda: {
        ...
        "A": ["세이버", "네로", "카르나"]  # '네로' moved to A, '란슬' moved to S
    })
    ...
    # Restricted servants - limited to max 2 total across all players
    # If more than 2 players choose from this pool, assassin survival phase triggers
    restricted_servants: Set[str] = field(default_factory=lambda: {
        "징어", "세미", "서문", "잭더리퍼", "무사시", "허새", "디미", "아엑", "셰익", "안데", "메데이아", "어벤저", "시키"
    })
    max_restricted_count: int = 2  # Maximum number of restricted servants allowed
    
    # Assassin survival phase tracking
    assassin_survival_players: List[int] = field(default_factory=list)  # Players who chose restricted servants
    assassin_survival_winners: List[int] = field(default_factory=list)  # Winners who keep their servants
    dishwashing_players: List[int] = field(default_factory=list)  # Losers who need to reselect
    assassin_survival_message_id: Optional[int] = None
    dishwashing_progress_message_id: Optional[int] = None
    dishwashing_start_time: Optional[float] = None
    dishwashing_time_limit: int = 90  # 1 minute 30 seconds
    ...
    reselection_time_limit: int = 130  # 1 minute 30 seconds
...
class TeamDraftCommands(BaseCommands):
    ...
    async def _handle_reselection_timeout(self, draft: DraftSession) -> None:
        ...
        # Get available servants (exclude confirmed, banned, restricted, and already selected)
        ...
        available_servants = list(draft.available_servants - confirmed_servants - draft.banned_servants - draft.restricted_servants - taken_servants)
    ...
    async def _create_status_embed(self, draft: DraftSession) -> discord.Embed:
        ...
        phase_names = {
            ...
            DraftPhase.ASSASSIN_SURVIVAL: "고독",
            DraftPhase.DISHWASHING: "잔반처리",
        ...
        if draft.phase == DraftPhase.ASSASSIN_SURVIVAL:
            embed.add_field(
                name="고독 참가자",
                value=f"{len(draft.assassin_survival_players)}명 참가 중",
                inline=True
            )
        
        if draft.phase == DraftPhase.DISHWASHING:
            remaining = len([uid for uid in draft.dishwashing_players 
                           if not draft.players[uid].selected_servant])
            embed.add_field(
                name="잔반처리 진행도",
                value=f"{len(draft.dishwashing_players) - remaining}/{len(draft.dishwashing_players)} 완료",
                inline=True
            )
    ...
    async def _reveal_servant_selections(self, draft: DraftSession = None) -> None:
        ...
        if conflicts:
            ...
            if any(current_draft.conflicted_servants.values()):
                ...
                # No more conflicts, check for restricted servants before team selection
                await self._check_restricted_servants_and_proceed(current_draft, current_channel_id)
        else:
            ...
            # Check for restricted servants before team selection
            await self._check_restricted_servants_and_proceed(current_draft, current_channel_id)
    ...
    async def _start_servant_reselection(self, draft: DraftSession, channel_id: int) -> None:
        ...
        # Show available characters in first category (exclude confirmed + banned + restricted)
        available_saber = [
            char for char in draft.servant_categories["세이버"] 
            if char not in taken_servants and char not in draft.banned_servants and char not in draft.restricted_servants
        ]
        if available_saber:
            embed.add_field(
                name="세이버 사용 가능",
        ...
        # Show restricted servants that are not available during reselection
        restricted_list = sorted(draft.restricted_servants)
        embed.add_field(
            name="🚫 제한 서번트 (재선택 불가)",
            value=", ".join(restricted_list),
            inline=False
        )
        ...
        embed.add_field(
            ...
            value="각자의 개인 선택 버튼을 사용해서 재선택해줘.\n"
                  "🔒 확정된 서번트, ❌ 금지된 서번트, 🚫 제한 서번트는 선택할 수 없어.",
            inline=False
        )
    ...
    async def _auto_complete_reselection(self, draft: DraftSession) -> None:
        ...
        # Get available servants (exclude confirmed, banned, and restricted)
        confirmed_servants = set(draft.confirmed_servants.values())
        available_servants = list(draft.available_servants - confirmed_servants - draft.banned_servants - draft.restricted_servants)
    ...
    async def _check_restricted_servants_and_proceed(self, draft: DraftSession, channel_id: int) -> None:
        """Check if too many restricted servants are selected, trigger assassin survival or proceed to team selection"""
        # Count players who selected restricted servants
        restricted_players = []
        for user_id, player in draft.players.items():
            servant = player.selected_servant or draft.confirmed_servants.get(user_id)
            if servant and servant in draft.restricted_servants:
                restricted_players.append(user_id)
        
        logger.info(f"Restricted servants check: {len(restricted_players)} players selected restricted servants (max: {draft.max_restricted_count})")
        
        if len(restricted_players) > draft.max_restricted_count:
            # Too many restricted servants, start assassin survival phase
            draft.assassin_survival_players = restricted_players
            await self._start_assassin_survival_phase(draft, channel_id)
        else:
            # Within limit, proceed to team selection
            await self._start_team_selection(draft, channel_id)

    async def _start_assassin_survival_phase(self, draft: DraftSession, channel_id: int) -> None:
        """Start assassin survival phase - dice roll to determine who keeps restricted servants"""
        draft.phase = DraftPhase.ASSASSIN_SURVIVAL
        
        channel = self._get_draft_channel(draft)
        if not channel:
            logger.warning(f"Could not get draft channel for assassin survival phase")
            return
        
        # Get the restricted servants that were selected
        restricted_selections = {}
        for user_id in draft.assassin_survival_players:
            player = draft.players[user_id]
            servant = player.selected_servant or draft.confirmed_servants.get(user_id)
            if servant:
                restricted_selections[user_id] = servant
        
        # Announce the phase
        embed = discord.Embed(
            title="🎲 고독",
            description=(
                f"**{len(draft.assassin_survival_players)}명**이 제한된 서번트를 선택했어!\n"
                f"최대 **{draft.max_restricted_count}명**만 제한 서번트를 가질 수 있어.\n\n"
                "주사위를 굴려서 누가 서번트를 유지할지 결정하자!"
            ),
            color=INFO_COLOR
        )
        
        # Filter out invalid player IDs (defensive - shouldn't happen normally)
        valid_survival_players = [uid for uid in draft.assassin_survival_players if uid in draft.players]
        if len(valid_survival_players) != len(draft.assassin_survival_players):
            invalid_ids = set(draft.assassin_survival_players) - set(valid_survival_players)
            logger.warning(f"Removed invalid player IDs from assassin_survival_players: {invalid_ids}")
            draft.assassin_survival_players = valid_survival_players
        
        # Show who selected what
        selections_text = "\n".join([
            f"• {draft.players[uid].username}: **{restricted_selections.get(uid, '?')}**"
            for uid in draft.assassin_survival_players
        ])
        if selections_text:
            embed.add_field(name="🎯 제한 서번트 선택자", value=selections_text, inline=False)
        else:
            logger.error("Assassin survival started but no valid players found!")
            # Skip to team selection if no valid players
            await self._start_team_selection(draft, channel_id)
            return
        
        # Roll dice for all players with tie-breaking
        rolls = {}
        max_attempts = 10
        attempt = 0
        remaining_players = draft.assassin_survival_players.copy()
        
        # Keep rolling until we can determine the top N winners
        while attempt < max_attempts:
            # Roll for all remaining players
            for user_id in remaining_players:
                rolls[user_id] = random.randint(1, 20)
            
            # Sort by roll value (descending)
            sorted_players = sorted(remaining_players, key=lambda uid: rolls[uid], reverse=True)
            
            # Check if we can clearly separate winners from losers
            # We need exactly max_restricted_count winners
            if len(sorted_players) <= draft.max_restricted_count:
                # Everyone remaining is a winner
                winners = sorted_players
                break
            
            # Find the cutoff roll value
            cutoff_idx = draft.max_restricted_count - 1
            cutoff_roll = rolls[sorted_players[cutoff_idx]]
            
            # Check for ties at the boundary
            players_at_cutoff = [uid for uid in sorted_players if rolls[uid] == cutoff_roll]
            players_above_cutoff = [uid for uid in sorted_players if rolls[uid] > cutoff_roll]
            
            if len(players_above_cutoff) >= draft.max_restricted_count:
                # Enough clear winners above cutoff
                winners = players_above_cutoff[:draft.max_restricted_count]
                break
            elif len(players_above_cutoff) + len(players_at_cutoff) == draft.max_restricted_count:
                # No tie at boundary - exact number
                winners = players_above_cutoff + players_at_cutoff
                break
            elif len(players_above_cutoff) < draft.max_restricted_count:
                # Need to resolve ties at cutoff
                spots_remaining = draft.max_restricted_count - len(players_above_cutoff)
                if len(players_at_cutoff) == spots_remaining:
                    # Exact match
                    winners = players_above_cutoff + players_at_cutoff
                    break
                else:
                    # Tie needs re-roll - only re-roll tied players
                    remaining_players = players_at_cutoff
                    logger.info(f"Assassin survival tie at roll {cutoff_roll}, re-rolling {len(remaining_players)} players (attempt {attempt + 1})")
                    attempt += 1
            else:
                winners = sorted_players[:draft.max_restricted_count]
                break
        
        # If still tied after max attempts, use deterministic tiebreaker
        if attempt >= max_attempts:
            sorted_by_roll_then_id = sorted(
                draft.assassin_survival_players,
                key=lambda uid: (-rolls.get(uid, 0), uid)
            )
            winners = sorted_by_roll_then_id[:draft.max_restricted_count]
            logger.warning(f"Assassin survival max re-rolls reached, using ID tiebreaker")
        
        # Determine losers
        losers = [uid for uid in draft.assassin_survival_players if uid not in winners]
        
        draft.assassin_survival_winners = winners
        draft.dishwashing_players = losers
        ...
        for user_id in draft.assassin_survival_players:
            ...
            if player:
                roll = rolls.get(user_id, 0)
                status = "✅" if user_id in winners else "❌"
                roll_results.append(f"{player.username}: **{roll}** {status}")
        ...
        if attempt > 0:
            embed.add_field(name="ℹ️ 재굴림", value=f"{attempt}회 재굴림 발생", inline=True)
        
        # Show winners
        winners_text = "\n".join([
            f"• {draft.players[uid].username} → **{restricted_selections.get(uid, '?')}** 유지"
            for uid in winners
            if uid in draft.players  # Defensive check
        ])
        if winners_text:
            embed.add_field(name="🏆 승자 (서번트 유지)", value=winners_text, inline=False)
        
        # Show losers
        losers_text = "\n".join([
            f"• {draft.players[uid].username} → 잔반처리 행"
            for uid in losers
            if uid in draft.players  # Defensive check
        ])
        if losers_text:
            embed.add_field(name="💧 패자 (잔반처리 행)", value=losers_text, inline=False)
        
        await self._safe_api_call(
            lambda: channel.send(embed=embed),
            bucket=f"assassin_survival_{channel_id}"
        )
        
        # Reset losers' selections - they need to pick new servants
        for user_id in losers:
            # Clear their confirmed servant if any
            if user_id in draft.confirmed_servants:
                del draft.confirmed_servants[user_id]
            # Clear their selected servant
            if user_id in draft.players:
                draft.players[user_id].selected_servant = None
                draft.selection_progress[user_id] = False
        
        # Confirm winners' selections
        for user_id in winners:
            servant = restricted_selections.get(user_id)
            if servant and user_id in draft.players:
                draft.confirmed_servants[user_id] = servant
        
        # Start dishwashing phase for losers
        if losers:
            await self._start_dishwashing_phase(draft, channel_id)
        else:
            # No losers (shouldn't happen normally), proceed to team selection
            await self._start_team_selection(draft, channel_id)

    async def _start_dishwashing_phase(self, draft: DraftSession, channel_id: int) -> None:
        """Start dishwashing phase - losers pick new servants from remaining pool"""
        draft.phase = DraftPhase.DISHWASHING
        
        channel = self._get_draft_channel(draft)
        if not channel:
            logger.warning(f"Could not get draft channel for dishwashing phase")
            return
        
        # Remove confirmed servants from available pool
        taken_servants = set(draft.confirmed_servants.values())
        draft.available_servants = draft.available_servants - taken_servants - draft.banned_servants
        
        logger.info(f"Dishwashing phase: {len(draft.dishwashing_players)} players need to reselect from {len(draft.available_servants)} available servants")
        
        embed = discord.Embed(
            title="💧 잔반처리 단계",
            description=(
                "고독에서 패배한 플레이어들은 남은 서번트 중에서 새로 선택해야 해!\n"
                f"⏰ **제한 시간: {draft.dishwashing_time_limit // 60}분 {draft.dishwashing_time_limit % 60}초**"
            ),
            color=INFO_COLOR
        )
        
        # Show who needs to reselect
        reselect_names = [draft.players[uid].username for uid in draft.dishwashing_players if uid in draft.players]
        if reselect_names:
            embed.add_field(name="🔄 재선택 대상", value="\n".join(reselect_names), inline=False)
        
        # Show confirmed servants (locked in)
        if draft.confirmed_servants:
            confirmed_chars = sorted(set(draft.confirmed_servants.values()))
            embed.add_field(
                name="✅ 확정된 서번트 (수정 불가)",
                value="\n".join([f"🔒 {s}" for s in confirmed_chars]),
                inline=False
            )
        
        # Show restricted servants that are not available during dishwashing
        restricted_list = sorted(draft.restricted_servants)
        embed.add_field(
            name="🚫 제한 서번트 (선택 불가)",
            value=", ".join(restricted_list),
        ...
        # Show available servants in first category (excluding restricted)
        available_saber = [
            char for char in draft.servant_categories["세이버"]
            if char not in taken_servants and char not in draft.banned_servants and char not in draft.restricted_servants
        ]
        ...
        embed.add_field(
            name="📋 선택 방법",
            value="각자의 개인 선택 버튼을 사용해서 새 서번트를 선택해줘.\n"
                  "🔒 확정된 서번트, ❌ 금지된 서번트, 🚫 제한 서번트는 선택할 수 없어.",
            inline=False
        )
        
        # Create selection interface (reuse the existing EphemeralSelectionView)
        view = DishwashingSelectionView(draft, self)
        await self._safe_api_call(
            lambda: channel.send(embed=embed, view=view),
            bucket=f"dishwashing_{channel_id}"
        )
        
        # Send progress message
        progress_embed = discord.Embed(
            title="📊 잔반처리 진행 상황",
            description="잔반처리 대상 플레이어들의 진행 상황을 실시간으로 표시해.",
            color=INFO_COLOR
        )
        
        # Add progress for dishwashing players
        progress_lines = []
        for user_id in draft.dishwashing_players:
            player = draft.players.get(user_id)
            if player:
                status = "✅ 완료" if player.selected_servant else "⏳ 대기 중"
                progress_lines.append(f"{player.username}: {status}")
        
        progress_embed.add_field(
            name="진행 상황",
            value="\n".join(progress_lines) if progress_lines else "진행 중...",
            inline=False
        )
        ...
        try:
            progress_message = await self._safe_api_call(
                lambda: channel.send(embed=progress_embed),
                bucket=f"dishwashing_progress_{channel_id}"
            )
            draft.dishwashing_progress_message_id = progress_message.id
            logger.info(f"Created dishwashing progress message {progress_message.id}")
        except Exception as e:
            logger.error(f"Failed to send dishwashing progress message: {e}")
        
        # Initialize dishwashing timer
        draft.dishwashing_start_time = time.monotonic()
        
        # Start background timer task
        task = asyncio.create_task(self._dishwashing_timer(draft))
        draft.running_tasks.add(task)
        task.add_done_callback(draft.running_tasks.discard)
        
        # Auto-complete for fake players in test mode
        if draft.is_test_mode:
            await self._auto_complete_dishwashing(draft)
            await self._update_dishwashing_progress_message(draft)

    async def _dishwashing_timer(self, draft: DraftSession) -> None:
        """Timer for dishwashing phase"""
        await asyncio.sleep(draft.dishwashing_time_limit)
        
        # Check if still in dishwashing phase
        if draft.phase != DraftPhase.DISHWASHING:
            return
        
        # Handle timeout
        await self._handle_dishwashing_timeout(draft)

    async def _handle_dishwashing_timeout(self, draft: DraftSession) -> None:
        """Handle timeout during dishwashing phase"""
        channel = self._get_draft_channel(draft)
        if not channel:
            return
        
        # Auto-assign servants to players who didn't select (exclude restricted servants)
        taken_servants = set(draft.confirmed_servants.values())
        available = list(draft.available_servants - taken_servants - draft.banned_servants - draft.restricted_servants)
        
        timeout_players = []
        for user_id in draft.dishwashing_players:
            player = draft.players.get(user_id)
            if player and not player.selected_servant:
                if available:
                    servant = random.choice(available)
                    player.selected_servant = servant
                    draft.confirmed_servants[user_id] = servant
                    available.remove(servant)
                    timeout_players.append(f"{player.username} → {servant}")
        
        if timeout_players:
            embed = discord.Embed(
                title="⏰ 잔반처리 시간 초과",
                description="시간 내에 선택하지 않은 플레이어들에게 자동으로 서번트가 배정됐어.",
                color=ERROR_COLOR
            )
            embed.add_field(name="자동 배정", value="\n".join(timeout_players), inline=False)
            await self._safe_api_call(
                lambda: channel.send(embed=embed),
                bucket=f"dishwashing_timeout_{draft.channel_id}"
            )
        
        # Proceed to team selection
        await self._start_team_selection(draft, draft.channel_id)

    async def _auto_complete_dishwashing(self, draft: DraftSession) -> None:
        """Auto-complete dishwashing for fake players in test mode"""
        taken_servants = set(draft.confirmed_servants.values())
        # Exclude restricted servants - dishwashing players can't pick them
        available = list(draft.available_servants - taken_servants - draft.banned_servants - draft.restricted_servants)
        
        for user_id in draft.dishwashing_players:
            player = draft.players.get(user_id)
            if player and user_id != draft.real_user_id and available:
                servant = random.choice(available)
                player.selected_servant = servant
                available.remove(servant)
                logger.info(f"Auto-selected {servant} for fake player {player.username} (dishwashing)")

    async def _update_dishwashing_progress_message(self, draft: DraftSession) -> None:
        """Update the dishwashing progress message"""
        if not draft.dishwashing_progress_message_id:
            return
        ...
        try:
            message = await channel.fetch_message(draft.dishwashing_progress_message_id)
            ...
            await self._safe_api_call(
                lambda: message.edit(embed=progress_embed),
                bucket=f"dishwashing_progress_update_{draft.channel_id}"
            )
        except Exception as e:
            logger.error(f"Failed to update dishwashing progress message: {e}")

    async def _check_dishwashing_completion(self, draft: DraftSession) -> None:
        """Check if dishwashing phase is complete and proceed if so"""
        # Prevent race condition - only proceed if still in dishwashing phase
        if draft.phase != DraftPhase.DISHWASHING:
            logger.info(f"Dishwashing completion check skipped - phase is {draft.phase.value}")
            return
        
        # Filter out invalid player IDs (defensive - shouldn't happen normally)
        valid_dishwashing_players = [uid for uid in draft.dishwashing_players if uid in draft.players]
        if len(valid_dishwashing_players) != len(draft.dishwashing_players):
            invalid_ids = set(draft.dishwashing_players) - set(valid_dishwashing_players)
            logger.warning(f"Removed invalid player IDs from dishwashing_players: {invalid_ids}")
            draft.dishwashing_players = valid_dishwashing_players
        
        # If no valid dishwashing players remain, skip to team selection
        if not draft.dishwashing_players:
            logger.warning("No valid dishwashing players remain, proceeding to team selection")
            await self._start_team_selection(draft, draft.channel_id)
            return
        
        # Check if all dishwashing players have selected
        all_selected = True
        for user_id in draft.dishwashing_players:
            player = draft.players.get(user_id)
            if not player or not player.selected_servant:
                all_selected = False
                break
        
        if all_selected:
            # Prevent double-execution by immediately changing phase
            draft.phase = DraftPhase.TEAM_SELECTION  # Mark as transitioning
            
            # Confirm all selections
            for user_id in draft.dishwashing_players:
                player = draft.players.get(user_id)
                if player and player.selected_servant:
                    draft.confirmed_servants[user_id] = player.selected_servant
            ...
            if channel:
                embed = discord.Embed(
                    title="✅ 잔반처리 완료",
                    description="모든 잔반처리 대상 플레이어들이 새 서번트를 선택했어!",
                    color=SUCCESS_COLOR
                )
                
                selections_text = "\n".join([
                    f"• {draft.players[uid].username}: **{draft.players[uid].selected_servant}**"
                    for uid in draft.dishwashing_players
                    if uid in draft.players and draft.players[uid].selected_servant
                ])
                if selections_text:
                    embed.add_field(name="선택 결과", value=selections_text, inline=False)
                
                await self._safe_api_call(
                    lambda: channel.send(embed=embed),
                    bucket=f"dishwashing_complete_{draft.channel_id}"
                )
    ...
    async def _perform_system_bans(self, draft: DraftSession, channel) -> None:
        ...
        logger.info(f"Available tiers - S: {len(available_s_tier)}, A: {len(available_a_tier)}")
...
class DishwashingSelectionView(discord.ui.View):
    """View with button for dishwashing players to open their private selection interface"""
    ...
    def __init__(self, draft: DraftSession, bot_commands: 'TeamDraftCommands'):
        ...
        # Add single button for dishwashing players
        button = DishwashingSelectionButton()
        self.add_item(button)


class DishwashingSelectionButton(discord.ui.Button):
    """Button for dishwashing players to open their private selection interface"""
    
    def __init__(self):
        super().__init__(
            label="💧 새 서번트 선택하기",
            style=discord.ButtonStyle.primary,
            custom_id="open_dishwashing_selection",
            emoji="🍽️",
            row=0
        )

    async def callback(self, interaction: discord.Interaction) -> None:
        """Open private selection interface for the dishwashing player"""
        try:
            user_id = interaction.user.id
            view: DishwashingSelectionView = self.view
            
            logger.info(f"Dishwashing selection button clicked by user {user_id}")
            ...
            # Validate user is in draft
            if user_id not in view.draft.players:
                logger.warning(f"User {user_id} not in draft but tried to access dishwashing interface")
                await interaction.response.send_message(
                    "드래프트 참가자만 선택 인터페이스를 사용할 수 있어.", ephemeral=True
                )
                return
            ...
            # Check if still in dishwashing phase
            if view.draft.phase != DraftPhase.DISHWASHING:
                await interaction.response.send_message(
                    "잔반처리 단계가 이미 종료됐어. 이 버튼은 더 이상 사용할 수 없어.", ephemeral=True
                )
                return
            
            # Check if user is a dishwashing player
            if user_id not in view.draft.dishwashing_players:
                # Allow test mode real user to access
                if not (view.draft.is_test_mode and user_id == view.draft.real_user_id):
                    player_name = view.draft.players[user_id].username
                    await interaction.response.send_message(
                        f"**{player_name}**은(는) 잔반처리 대상이 아니야.\n"
                        "고독에서 패배한 플레이어만 새 서번트를 선택할 수 있어.", ephemeral=True
                    )
                    return
            ...
            if current_selection:
                logger.info(f"Player {user_id} ({player_name}) already completed dishwashing selection: {current_selection}")
                await interaction.response.send_message(
                    f"이미 새 서번트를 선택했어: **{current_selection}**\n"
                    "더 이상 변경할 수 없어.", 
                    ephemeral=True
                )
                return
            
            # Open private selection interface (reuse existing PrivateSelectionView)
            logger.info(f"Opening dishwashing selection interface for player {user_id} ({player_name})")
            private_view = PrivateSelectionView(view.draft, view.bot_commands, user_id)
            
            await interaction.response.send_message(
                f"**{player_name}의 잔반처리 선택**\n"
                "남은 서번트 중 원하는 서번트를 한 명 선택해줘.\n"
                "이미 선택된 서번트와 금지된 서번트는 선택할 수 없어.",
                ephemeral=True,
                view=private_view
            )
            logger.info(f"Successfully sent ephemeral dishwashing selection interface to player {user_id} ({player_name})")
            
        except Exception as e:
            logger.error(f"Error in dishwashing selection button callback: {e}", exc_info=True)
            try:
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        "선택 인터페이스 열기에 실패했어. 다시 시도해줘.", ephemeral=True
...
class PrivateSelectionView(discord.ui.View):
    ...
    def _add_character_dropdown(self):
        ...
        # During reselection or dishwashing, also exclude confirmed servants AND restricted servants
        if self.draft.phase in (DraftPhase.SERVANT_RESELECTION, DraftPhase.DISHWASHING):
            excluded_servants.update(self.draft.confirmed_servants.values())
            excluded_servants.update(self.draft.restricted_servants)  # Can't pick restricted servants during reselection/dishwashing
    ...
    async def update_category(self, new_category: str, interaction: discord.Interaction):
        ...
        for char in chars_in_category:
            ...
            elif char in self.draft.confirmed_servants.values() and self.draft.phase in (DraftPhase.SERVANT_RESELECTION, DraftPhase.DISHWASHING):
                char_list.append(f"🔒 {char}")
            elif char in self.draft.restricted_servants and self.draft.phase in (DraftPhase.SERVANT_RESELECTION, DraftPhase.DISHWASHING):
                char_list.append(f"🚫 {char} (제한)")
            else:
                char_list.append(f"• {char}")
...
class PrivateSelectionCategoryButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if view.draft.phase not in [DraftPhase.SERVANT_SELECTION, DraftPhase.SERVANT_RESELECTION, DraftPhase.DISHWASHING]:
            await interaction.response.send_message(
        ...
        # 2. Completion validation - prevent changes after confirmation (for initial selection only)
        # During dishwashing, we use selected_servant instead of selection_progress
        if view.draft.phase == DraftPhase.DISHWASHING:
            player = view.draft.players.get(view.user_id)
            if player and player.selected_servant:
                await interaction.response.send_message(
                    "이미 새 서번트를 선택했어. 더 이상 변경할 수 없어.", 
                    ephemeral=True
                )
                return
        elif view.draft.selection_progress.get(view.user_id, False):
            await interaction.response.send_message(
        ...
        # 4. Dishwashing validation - during dishwashing, only allow dishwashing players
        if view.draft.phase == DraftPhase.DISHWASHING:
            if view.user_id not in view.draft.dishwashing_players and not (view.draft.is_test_mode and user_id == view.draft.real_user_id):
                player = view.draft.players.get(view.user_id)
                user_name = player.username if player else "Unknown"
                await interaction.response.send_message(
                    f"**{user_name}**은(는) 잔반처리 대상이 아니야.\n"
                    "고독에서 패배한 플레이어만 새 서번트를 선택할 수 있어.", ephemeral=True
                )
                return
...
class ConfirmSelectionButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        # During dishwashing phase, only allow dishwashing players to confirm
        if view.draft.phase == DraftPhase.DISHWASHING:
            # In test mode, allow real user to confirm for any user for testing
            if self.user_id not in view.draft.dishwashing_players and not (view.draft.is_test_mode and user_id == view.draft.real_user_id):
                await interaction.response.send_message(
                    f"**{user_name}**은(는) 잔반처리 대상이 아니야.\n"
                    "고독에서 패배한 플레이어만 새 서번트를 선택할 수 있어.", ephemeral=True
                )
        ...
        # During reselection or dishwashing, ensure servant isn't already confirmed by someone else
        if view.draft.phase in (DraftPhase.SERVANT_RESELECTION, DraftPhase.DISHWASHING):
            if view.selected_servant in view.draft.confirmed_servants.values():
                logger.warning(f"User {self.user_id} ({user_name}) tried to confirm already taken servant during {view.draft.phase.value}: {view.selected_servant}")
                await interaction.response.send_message(
            ...
            # During reselection or dishwashing, restricted servants are not allowed
            if view.selected_servant in view.draft.restricted_servants:
                logger.warning(f"User {self.user_id} ({user_name}) tried to confirm restricted servant during {view.draft.phase.value}: {view.selected_servant}")
                await interaction.response.send_message(
                    f"**{view.selected_servant}**은(는) 제한된 서번트야. 재선택/잔반처리 단계에서는 선택할 수 없어.",
                    ephemeral=True
                )
                view.selected_servant = None  # Reset invalid selection
                return
        ...
        elif view.draft.phase == DraftPhase.DISHWASHING:
            # During dishwashing: update progress and check completion
            await view.bot_commands._update_dishwashing_progress_message(view.draft)
            await view.bot_commands._check_dishwashing_completion(view.draft)
        else:
...
class PrivateSelectionCharacterDropdown(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction) -> None:
        ...
        if view.draft.phase == DraftPhase.DISHWASHING:
            # During dishwashing, check if servant is already selected
            if view.draft.players[self.user_id].selected_servant:
                await interaction.response.send_message(
                    "이미 새 서번트를 선택했어. 더 이상 변경할 수 없어.", 
                    ephemeral=True
                )
                return
        elif view.draft.selection_progress.get(self.user_id, False):
            await interaction.response.send_message(
                "이미 선택을 완료했어. 더 이상 변경할 수 없어.", 
                ephemeral=True
            )
